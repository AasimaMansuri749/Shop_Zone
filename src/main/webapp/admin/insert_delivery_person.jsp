...
<body class="bg-light">
    <div class="container mt-3">
        ...
        <form action="" method="post" onsubmit="return validateForm()" autocomplete="off">
            ...
            <div class="form-outline mb-4 w-50 m-auto">
                <label for="order_area1" class="form-label">Order Area 1:</label>
                <select name="order_area1" id="order_area1" class="form-control" required>
                    <option value="">Select an area</option>
                    <% for (String[] area : areas) { %>
                        <option value="<%= area[2] %>"><%= area[1] %> ( <%= area[2] %>)</option>
                    <% } %>
                </select>
            </div>
            <div class="form-outline mb-4 w-50 m-auto">
                <label for="order_area2" class="form-label">Order Area 2:</label>
                <select name="order_area2" id="order_area2" class="form-control" required>
                    <option value="">Select an area</option>
                    <% for (String[] area : areas) { %>
                        <option value="<%= area[2] %>"><%= area[1] %> ( <%= area[2] %>)</option>
                    <% } %>
            ...
            <div class="form-outline mb-4 w-50 m-auto">
                <input type="submit" value="Insert Delivery Person" class="btn btn-info mb-3 px-3">
            </div>
        </form>
    </div>
</body>
